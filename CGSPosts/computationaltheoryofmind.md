...
In this sense, connectionism is in stark contrast to the classical view: there are no rules, representations, or anything else. There are only the neurons and their weights. "Following a rule", for example, would instead be represented as a neural network outputting in a pattern which we recognize as conforming (at least approximately) to a rule -- for something like $ 2 + 2$ would be considered solved if the network outputs 4, not because it represented the numbers and manipulated them according to the rule of addition. 
